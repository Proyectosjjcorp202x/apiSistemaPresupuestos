...
if (isset($postdata) && !empty($postdata)) {
    ...
    // ****************************
    // Armado de consulta
    $sqlConsulta = "SELECT cs.* FROM vw_cargas_sociales cs "
            . "     WHERE cs.activo = 1 ";
    $sqlCond = " vw_cargas_sociales cs "
            . "     WHERE cs.activo = 1";
    if (strlen($cond) > 0) {
        $sqlConsulta .= " "
                . " AND (trim(lower(cs.descripcion)) LIKE trim(lower('" . $cond . "%'))"
                . " OR cs.valor like '" . $cond . "%')";
        $sqlCond .= " AND (trim(lower(cs.descripcion)) LIKE trim(lower('" . $cond . "%'))"
                . " OR cs.valor like '" . $cond . "%')";
    }
    ...
    $cargas_sociales = $mysql->Query($sqlConsulta, $Select_Type);
