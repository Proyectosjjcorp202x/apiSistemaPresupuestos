...
if (isset($postdata) && !empty($postdata)) {
    ...
    // ****************************
    // Armado de consulta
    $sqlConsulta = "SELECT p.* FROM vw_proveedores p "
            . "     WHERE p.activo = 1 ";
    $sqlCond = " vw_proveedores p "
            . "     WHERE p.activo = 1";
    if (strlen($cond) > 0) {
        $sqlConsulta .= " "
                . " AND trim(lower(p.nombre)) LIKE trim(lower('" . $cond . "%'))";
        $sqlCond .= " AND trim(lower(p.nombre)) LIKE trim(lower('" . $cond . "%'))";
    }
    ...
    $proveedores = $mysql->Query($sqlConsulta, $Select_Type);
