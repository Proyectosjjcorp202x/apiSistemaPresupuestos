...
if (isset($postdata) && !empty($postdata)) {
    ...
    // ****************************
    // Armado de consulta
    $sqlConsulta = "SELECT c.* FROM vw_catodiv c "
            . "     WHERE c.activo = 1 ";
    $sqlCond = " vw_catodiv c "
            . "     WHERE c.activo = 1";
    if (strlen($cond) > 0) {
        $sqlConsulta .= " "
                . " AND trim(lower(c.cat_o_div)) LIKE trim(lower('" . $cond . "%'))";
        $sqlCond .= " AND trim(lower(c.cat_o_div)) LIKE trim(lower('" . $cond . "%'))";
    }
    ...
    $cat_o_div = $mysql->Query($sqlConsulta, $Select_Type);
