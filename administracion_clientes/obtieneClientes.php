...
if (isset($postdata) && !empty($postdata)) {
    ...
    // ****************************
    // Armado de consulta
    $sqlConsulta = "SELECT c.* FROM vw_clientes c "
            . "     WHERE c.activo = 1 ";
    $sqlCond = " vw_clientes c "
            . "     WHERE c.activo = 1";
    if (strlen($cond) > 0) {
        $sqlConsulta .= " "
                . " AND trim(lower(c.nombre)) LIKE trim(lower('" . $cond . "%'))";
        $sqlCond .= " AND trim(lower(c.nombre)) LIKE trim(lower('" . $cond . "%'))";
    }
