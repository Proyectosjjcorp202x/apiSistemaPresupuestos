...
if (isset($postdata) && !empty($postdata)) {
    ...
    // ****************************
    // Armado de consulta
    $sqlConsulta = "SELECT p.* FROM vw_plazas p "
            . "     WHERE p.activo = 1 ";
    $sqlCond = " vw_plazas p "
            . "     WHERE p.activo = 1";
    if (strlen($cond) > 0) {
        $sqlConsulta .= " "
                . " AND (trim(lower(p.clave_depto)) LIKE trim(lower('" . $cond . "%')) OR "
                . " trim(lower(p.descripcion))  LIKE trim(lower('" . $cond . "%')))";
        $sqlCond .= " AND (trim(lower(p.clave_depto)) LIKE trim(lower('" . $cond . "%')) OR "
                . " trim(lower(p.descripcion))  LIKE trim(lower('" . $cond . "%')))";
    }
    ...
    $plazas = $mysql->Query($sqlConsulta, $Select_Type);
