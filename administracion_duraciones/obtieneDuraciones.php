...
if (isset($postdata) && !empty($postdata)) {
    ...
    // ****************************
    // Armado de consulta
    $sqlConsulta = "SELECT d.* FROM vw_duraciones d "
            . "     WHERE d.activo = 1 ";
    $sqlCond = " vw_duraciones d "
            . "     WHERE d.activo = 1";
    if (strlen($cond) > 0) {
        $sqlConsulta .= " "
                . " AND (trim(lower(d.duracion)) LIKE trim(lower('" . $cond . "%'))"
                . " OR d.dias like '" . $cond . "%')";
        $sqlCond .= " AND (trim(lower(d.duracion)) LIKE trim(lower('" . $cond . "%'))"
                . " OR d.dias like '" . $cond . "%')";
    }
    ...
    $duraciones = $mysql->Query($sqlConsulta, $Select_Type);
