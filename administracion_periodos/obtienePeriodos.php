...
if (isset($postdata) && !empty($postdata)) {
    ...
    // ****************************
    // Armado de consulta
    $sqlConsulta = "SELECT p.* FROM vw_periodos p "
            . "     WHERE p.activo = 1 ";
    $sqlCond = " vw_periodos p "
            . "     WHERE p.activo = 1";
    if (strlen($cond) > 0) {
        $sqlConsulta .= " "
                . " AND trim(lower(p.periodo)) LIKE trim(lower('" . $cond . "%'))";
        $sqlCond .= " AND trim(lower(p.periodo)) LIKE trim(lower('" . $cond . "%'))";
    }
    ...
    $periodos = $mysql->Query($sqlConsulta, $Select_Type);
