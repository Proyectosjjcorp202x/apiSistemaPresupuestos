...
if (isset($postdata) && !empty($postdata)) {
    ...
    // ****************************
    // Armado de consulta
    $sqlConsulta = "SELECT u.* FROM vw_usuarios u "
            . "     WHERE u.activo = 1 ";
    $sqlCond = " vw_usuarios u "
            . "     WHERE u.activo = 1";
    if (strlen($cond) > 0) {
        $sqlConsulta .= " "
                . " AND (trim(lower(u.usuario)) LIKE trim(lower('" . $cond . "%')) "
                . " OR trim(lower(u.perfil)) LIKE trim(lower('" . $cond . "%'))  
                    OR trim(lower(u.nombrecompleto)) LIKE trim(lower('%" . $cond . "%')))";
        $sqlCond .= " AND (trim(lower(u.usuario)) LIKE trim(lower('" . $cond . "%')) "
                . " OR trim(lower(u.perfil)) LIKE trim(lower('" . $cond . "%'))  
                    OR trim(lower(u.nombrecompleto)) LIKE trim(lower('%" . $cond . "%')))";
    }
    ...
    $usuarios = $mysql->Query($sqlConsulta, $Select_Type);
